...
async def test_get_account_by_login_password_success(account_repo: AccountRepository):
    ...
    await account_repo.create_account("test_auth_ok", "123456")

    result = await account_repo.get_account_by_login_password("test_auth_ok", "123456")

    assert result is not None
    assert result["login"] == "test_auth_ok"
...
async def test_get_account_by_login_password_wrong_password(
    ...
    await account_repo.create_account("test_auth_bad_pass", "123456")

    result = await account_repo.get_account_by_login_password(
        "test_auth_bad_pass", "123456789"
    )
