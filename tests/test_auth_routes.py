...
async def test_login_wrong_password(client, account_repo):
    ...
    response = client.post("/login", json={"login": login, "password": "wrong"})
...
async def test_login_user_not_found(client):
    ...
    response = client.post(
        "/login", json={"login": f"nonexistent_{uuid.uuid4()}", "password": "any"}
    )
    assert response.status_code == 401
...
async def test_login_empty_credentials(client):
    # пустые креды вызывают ошибку валидации Pydantic

    response = client.post("/login", json={"login": "", "password": ""})
...
async def test_login_sets_cookie_attributes(client, account_repo):
    ...
    response = client.post("/login", json={"login": login, "password": "pass"})
